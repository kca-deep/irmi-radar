  ...
  return (
    <div className="w-full px-[40px] pt-[32px] shrink-0 flex justify-center z-50">
      <div className="w-full max-w-[1440px] flex justify-between items-end pb-[20px]">
        <h1
          className="text-[34px] font-[900] text-[var(--irumi-text-1)] leading-none tracking-[-0.03em]"
        ...
        <div className="flex items-center gap-4">
          <div className="text-[14px] text-[var(--irumi-text-3)]">
            {/* 날짜는 서버 측에서 주입하거나 JS Date 사용 */}
            2026년 3월 14일 토요일 기준
          </div>
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="flex items-center justify-between gap-2 bg-transparent hover:bg-[#EBEBEB] text-[var(--irumi-text-2)] text-[13px] font-[600] px-3 py-2 rounded-[8px] transition-colors cursor-pointer min-w-[110px]"
            >
              {period}
              <svg
            ...
            {isDropdownOpen && (
              <div className="absolute top-[110%] right-0 w-full bg-card border border-border rounded-[8px] shadow-[0_4px_16px_rgba(0,0,0,0.1)] z-[100] py-1 overflow-hidden">
                {periods.map((p) => (
                  <button
                    key={p}
                    ...
                    className={`w-full text-left px-3 py-2 text-[13px] font-medium transition-colors hover:bg-[#FAFAFA] ${
                      period === p ? "text-irumi-brand bg-irumi-brand-muted" : "text-[var(--irumi-text-2)]"
                    }`}
                  >
                    {p}
                  </button>
          ...
          {/* 리포트 다운로드 */}
          <button className="flex items-center gap-2 bg-transparent hover:bg-[#EBEBEB] text-[var(--irumi-text-2)] text-[13px] font-[600] px-4 py-2 rounded-[8px] transition-colors cursor-pointer ml-1">
...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  return (
    <PeriodProvider>
      <div className="min-h-screen bg-irumi-page font-sans text-foreground flex w-full">
        <IrumiSidebar />
