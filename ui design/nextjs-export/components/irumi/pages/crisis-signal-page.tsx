...
export function CrisisSignalPage({ data }: CrisisSignalPageProps) {
  ...
  const headlineColor   = riskColorMap[topRisk] ?? "var(--irumi-caution)";
    ...
    <div className="px-[0px] pt-[0px] pb-[10px]">
      ...
      <div className="mt-[16px] mb-[20px] flex flex-col">
        <div className="flex items-center">
          <h1 className="text-[22px] font-[900] tracking-[-0.5px] text-[var(--irumi-text-1)]">
          ...
          <div className="ml-auto flex items-center gap-[10px]">
            ...
            <div className="flex items-center gap-[6px] bg-irumi-urgent-muted text-irumi-urgent px-[14px] py-[8px] rounded-[10px] border border-irumi-urgent-border shadow-sm">
              <span className="w-[8px] h-[8px] rounded-full bg-irumi-urgent animate-pulse" />
              <span className="text-[14px] font-[800]">긴급 {emergencyCount}</span>
            </div>
            <div className="flex items-center gap-[6px] bg-irumi-caution-muted text-irumi-caution px-[14px] py-[8px] rounded-[10px] border border-irumi-caution-border shadow-sm">
              <span className="w-[8px] h-[8px] rounded-full bg-irumi-caution" />
              <span className="text-[14px] font-[800]">주의 {cautionCount}</span>
            </div>
            <div className="flex items-center gap-[6px] bg-irumi-watch-muted text-irumi-watch px-[14px] py-[8px] rounded-[10px] border border-irumi-watch-border shadow-sm">
              <span className="w-[8px] h-[8px] rounded-full bg-irumi-watch" />
              <span className="text-[14px] font-[800]">관찰 {watchCount}</span>
            </div>
          </div>
        </div>
        <p className="text-[11px] text-[var(--irumi-text-3)] mt-[4px]">
          감지된 위기 신호와 지역별 위험도를 확인하세요
        </p>
      </div>

      {/* 필터 바 */}
      <div className="h-[52px] bg-card rounded-[14px] shadow-[0_2px_10px_rgba(0,0,0,0.05)] px-[20px] flex items-center gap-[6px] mb-[20px] relative overflow-hidden">
            ...
            <button
              ...
              className={`text-[13px] px-[14px] py-[6px] rounded-[8px] border-none cursor-pointer transition-colors flex items-center gap-[6px] ${
                activeCategory === category
                  ? "bg-irumi-brand text-white font-[700]"
                  : "bg-transparent text-[var(--irumi-text-2)] hover:bg-[#F5F5F5] hover:text-[var(--irumi-text-1)]"
              }`}
            >
        ...
        <div className="relative">
          <select
            ...
            className={`appearance-none border rounded-[8px] text-[13px] py-[6px] pl-[14px] pr-[32px] outline-none cursor-pointer transition-colors ${
              filterRegion !== "전체"
                ? "bg-irumi-brand text-white font-[700] border-irumi-brand"
        ...
        {(filterRegion !== "전체" || filterRisk !== "전체") && (
          <button
            ...
            className="ml-2 text-[12px] text-[var(--irumi-text-3)] hover:text-irumi-brand transition-colors px-[8px] py-[6px] rounded-[6px] hover:bg-irumi-brand-muted"
          >
            ✕ 초기화
          </button>
        )}
      ...
      <div className="grid grid-cols-[660px_1fr] gap-[20px]">
        {/* 좌: 지도 */}
        <div className="bg-card rounded-[16px] shadow-[var(--irumi-shadow-card)] p-[24px] flex flex-col h-full">
          <div className="flex items-center justify-between mb-[16px]">
            <div className="flex items-center gap-[8px]">
              <div className="w-[6px] h-[6px] bg-irumi-brand rounded-full" />
              <h2 className="text-[16px] font-[700] text-[var(--irumi-text-1)]">지역별 위험도</h2>
            </div>
            <div className="text-[12px] text-[var(--irumi-text-3)]">지역 클릭 시 세부 확인</div>
          </div>

          <div className="bg-[#FAFAFA] rounded-[14px] overflow-hidden flex-shrink-0 cursor-pointer border border-irumi-line h-[560px] flex items-center justify-center">
          ...
          <div className="flex flex-col gap-[8px] mt-[16px]">
            <div className="flex items-center gap-[8px]">
              <span className="text-[11px] text-[var(--irumi-text-3)] font-[600] shrink-0">막대 순서</span>
              <span className="text-[11px] text-[#BBBBBB]">물 · 자 · 부 · 고 · 금&nbsp;&nbsp;(고정)</span>
            </div>
            <div className="flex items-center gap-[14px]">
              <div className="flex items-center gap-[6px]">
                <span className="text-[11px] text-[var(--irumi-text-3)] font-[600]">막대 색</span>
                {[
        ...
        <div className="flex flex-col gap-[20px] h-full">
          <div className="grid grid-cols-2 gap-[16px] shrink-0">
            ...
            <div className="rounded-[14px] p-[16px_18px] relative overflow-hidden" style={{ background: cardGradient }}>
              <div className="absolute inset-0 rounded-[14px] overflow-hidden pointer-events-none">
                <div className="absolute right-0 bottom-0 w-[130px] h-[130px] opacity-[0.12] pointer-events-none translate-x-[20%] translate-y-[20%]">
                  <Image src="/images/irumi-logo.png" alt="" width={130} height={130} className="object-contain" style={{ filter: "brightness(0) invert(1)" }} />
                </div>
              </div>
            ...
            {/* 위험 지역 TOP 5 */}
            <div className="bg-card rounded-[16px] p-[20px_22px] border border-irumi-line">
              <div className="flex items-center justify-between mb-[16px]">
                <div className="flex items-center gap-[8px]">
                  <div className="w-[6px] h-[6px] bg-irumi-brand rounded-full" />
                  <span className="text-[15px] font-[700] text-[var(--irumi-text-1)]">위험 지역 TOP 5</span>
                </div>
                ...
                {[...regions]
                  ...
                  .map((region, idx) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`text-[13px] font-[800] w-[16px] text-center shrink-0 ${idx === 0 ? "text-irumi-urgent" : idx === 1 ? "text-irumi-brand" : "text-[#CCCCCC]"}`}>{idx + 1}</span>
                        <span className="text-[13px] font-[600] text-[var(--irumi-text-2)] group-hover:text-irumi-brand transition-colors w-[32px] shrink-0">{region.name}</span>
          ...
          <div className="flex flex-col gap-[16px] flex-1 relative">
            <div className="flex items-center gap-[10px] shrink-0">
              <div className="flex-1 h-[1px] bg-irumi-line" />
              <span className="text-[13px] text-[#BBBBBB] font-[600] shrink-0">감지된 신호 {filteredSignals.length}건</span>
              <div className="flex-1 h-[1px] bg-irumi-line" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-[12px] flex-1 auto-rows-fr">
              {displayedSignals.length > 0 ? (
                ...
                <div className="col-span-2 py-[50px] flex flex-col items-center justify-center gap-[14px] bg-card rounded-[14px] border border-irumi-line">
                  <p className="text-[14px] font-[700] text-[#555555]">조건에 맞는 신호가 없어요</p>
                  <p className="text-[12px] text-[var(--irumi-text-3)]">필터를 변경하거나 초기화해 보세요</p>
                  <button onClick={() => { setFilterRegion("전체"); setFilterRisk("전체"); setActiveCategory("전체"); }} className="mt-[4px] text-[12px] font-[600] text-irumi-brand hover:underline">
                    필터 초기화
                  </button>
                </div>
              )}
            ...
            {filteredSignals.length > visibleCount && (
              <div className="absolute top-full left-0 w-full pt-[12px] flex justify-center">
                <button
                  ...
                  className="flex flex-col items-center justify-center text-[#CCCCCC] hover:text-irumi-brand transition-colors group"
                >
