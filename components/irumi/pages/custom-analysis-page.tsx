...
type IconData = typeof BankIcon;
import type { CustomAnalysisData, CustomCard, RiskGrade } from "@/lib/irumi/types";

// ── 카테고리 설정 ─────────────────────────────────────────
const CATEGORY_CONFIG: Record<string, { bg: string; icon: IconData; iconColor: string }> = {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", bottom: "-18px", right: "-18px", opacity: 0.16, pointerEvents: "none", zIndex: 0 }}>
        <HugeiconsIcon icon={cat.icon} size={110} color="white" />
      </div>
  ...
  return (
    <div className="pb-10 pt-[8px]">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between px-[28px] py-[20px] rounded-[24px] mb-[24px] relative overflow-hidden bg-white border border-[#EAEAEA] shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
        <div className="absolute top-0 bottom-0 right-0 w-[55%] bg-[#FFF6F0] z-0 hidden md:block" style={{ clipPath: "polygon(15% 0, 100% 0, 100% 100%, 0% 100%)" }} />
        ...
        <div className="absolute left-[406.5px] top-[10.5px] w-[120px] z-10 pointer-events-none hidden lg:block opacity-95">
          <img
            ...
            alt=""
            className="w-full h-auto drop-shadow-md mix-blend-multiply scale-110 origin-top"
          />
        </div>
        ...
        <div className="flex items-center gap-[16px] relative z-20 mb-4 md:mb-0">
          ...
          <div className="flex flex-col justify-center">
            <div className="text-[20px] font-[800] text-[#1A1A1A] tracking-[-0.5px] mb-[2px]">{data.userName}님, 반갑습니다!</div>
            <div className="text-[13px] text-[#777777] font-[500] tracking-tight">나에게 꼭 맞는 맞춤 혜택을 꼼꼼하게 확인해 보세요</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-[10px] relative z-20 bg-[#FFF6F0] py-[8px] px-[12px] rounded-[16px] border border-[#FFEADB]">
          ...
          <div className="relative w-[110px] h-[36px]">
            <select value={ageGroup} onChange={(e) => setAgeGroup(e.target.value)}
              className="w-full h-full appearance-none bg-white border border-[#E0D0C0] hover:border-[#FF6600] transition-colors rounded-[8px] text-[13px] text-[#333333] font-[600] pl-[12px] pr-[30px] outline-none shadow-sm cursor-pointer focus:ring-2 focus:ring-[#FF6600]/20">
          ...
          <button
            ...
            className="h-[36px] px-[18px] bg-[#FF6600] text-white text-[13px] font-[700] rounded-[8px] shadow-[0_4px_12px_rgba(255,102,0,0.25)] border-none hover:bg-[#E65C00] hover:-translate-y-[1px] active:translate-y-[0px] transition-all"
          >
            재분석
          </button>
        </div>
        ...
        <main className="w-full">
          <div className="mb-[14px]">
            <h1 className="text-[22px] font-[900] tracking-[-0.5px] leading-[1.3] text-[#1A1A1A]">
              <span className="text-[#FF6600]">{ageGroup} {jobGroup}</span>, 지금 당장 이것만 하세요.
            </h1>
                              ...
                              <div className="flex-1 pl-[20px] flex flex-col h-full">
                                ...
                                <div className="flex flex-col gap-2 overflow-y-auto max-h-[160px] pr-1">
                                  {newsItems.length > 0 ? newsItems.map((news, i) => (
                                    ...
                                    <div className="text-[12px] text-[#888888] py-4">관련 뉴스를 불러오는 중...</div>
                                  )}
                                </div>
                              </div>
                            </div>
            ...
            {!selectedCard && (
              ...
              >
                <div className="w-16 h-16 bg-[#F5F5F5] rounded-full flex items-center justify-center mb-4 shadow-sm">
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-[#AAAAAA]" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M9 9l5 12 1.774-5.226L21 14 9 9z"/>
                    <path d="M16.071 16.071l4.243 4.243"/>
                    <path d="m7.188 2.239.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122M5.98 11.964l-2.121 2.121"/>
                  </svg>
                </div>
                <div className="text-[15px] font-[700] text-[#444444] mb-2">위 카드 중 하나를 선택해보세요</div>
                <div className="text-[13px] text-[#888888] font-medium">카드를 클릭하면 상세한 지원 혜택과 신청 방법을 확인할 수 있습니다.</div>
              </motion.div>
            )}
          </AnimatePresence>
        </main>
