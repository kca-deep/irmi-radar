...
│   ├── signals/                 # 위기 신호 탭 페이지 (지역 지도 통합)
│   └── news/                    # 뉴스 분석 탭 페이지
├── components/
│   ├── ui/                      # shadcn 기본 UI 컴포넌트
│   ├── layout/                  # 공통 레이아웃 (헤더, 네비게이션, 탭)
│   ├── dashboard/               # 대시보드 위젯
│   ├── signals/                 # 위기 신호 관련 컴포넌트 (region-map/ 포함)
│   └── news/                    # 뉴스 분석 관련 컴포넌트
├── lib/
│   ├── utils.ts                 # cn() 유틸리티 (기존)
...
- **지역 지도 통합**: 17개 시도 지역별 위기 현황 지도 (react-simple-maps)
  - 지역별 위기 등급 색상 표시
  - 지역 클릭 시 해당 지역 신호 목록 필터링
...
- 위기 신호 탭: 신호 목록, 필터, 등급 배지, 상세 보기, 지역 지도 (react-simple-maps)
