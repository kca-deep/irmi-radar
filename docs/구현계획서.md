...
Phase 3: 대시보드 탭 UI + AI 브리핑 패널
    |
Phase 4: 위기 신호 탭 UI + 지역 지도 (지역별 현황 통합)
    |
Phase 5: 뉴스 분석 탭 UI
    |
Phase 6: 차별화 기능 UI (AI 대화형 질의 + 위기 연쇄 반응 맵)
    |
Phase 7: API Route 스캐폴딩 (Mock 반환)
    |
Phase 8: [해커톤 당일] 실데이터 교체 + Claude API 연결
```
...
| 지역 지도 | 위기 신호 탭에서 17개 시도 지역별 위기 현황을 지도로 시각화 | 지역 맞춤형 서비스 어필 | 사전 완전 구현 |
...
│   ├── page.tsx                  # 대시보드 (기본)
│   ├── signals/
│   │   └── page.tsx              # 위기 신호 (지역 지도 통합)
│   └── news/
│       └── page.tsx              # 뉴스 분석
├── api/                          # API Routes (Phase 6)
```
...
**목표**: 뉴스 분석 탭 화면 구현 + AI 분석 제어/진행 모달 UI
...
app/(tabs)/news/page.tsx
  └── components/news/news-page.tsx ("use client")
        ├── analysis-control-panel.tsx (분석 제어 패널)
        │     ├── 기간 선택: 프리셋 토글 (1주/1개월/3개월/6개월/1년/전체/직접설정)
        │     ├── 직접 설정 시: 시작일/종료일 Input (date)
        │     ├── 카테고리 선택: 토글 버튼 (전체 + 5대 카테고리)
        │     ├── 분석 정보: 대상 건수 + 예상 소요 시간 실시간 표시
        │     └── "AI 분석 시작하기" CTA 버튼
        │
        ├── analysis-progress-modal.tsx (분석 진행 모달)
        │     ├── 전체 진행률 Progress 바 (%)
        │     ├── 처리 건수 / 전체 건수
        │     ├── 7단계 체크리스트 (데이터수집 → 5개 카테고리 → 종합산출)
        │     ├── 현재 작업 텍스트 + 경과/잔여 시간
        │     ├── 분석 취소 버튼
        │     └── 완료 시: 결과 요약 + 네비게이션 버튼
        │
...
- `analysisPeriod`: 분석 기간 프리셋 키 또는 커스텀 범위
- `analysisCategories`: 분석 대상 카테고리 배열
- `analysisState`: 분석 상태 (idle/running/completed/error)
- 키워드 검색: 제목 + 키워드 배열에서 부분 일치 필터
- 카테고리 필터: category 필드 일치 필터
- 기간/카테고리 선택 시 하단 뉴스 목록 자동 필터링 연동

**분석 처리 파이프라인 (7단계)**:

| 단계 | 이름 | 설명 |
|------|------|------|
| 1 | 데이터 수집 | 선택 기간/카테고리의 기사 로드 |
| 2 | 물가 분석 | 물가 카테고리 기사 배치 분석 (Claude API) |
| 3 | 고용 분석 | 고용 카테고리 기사 배치 분석 |
| 4 | 자영업 분석 | 자영업 카테고리 기사 배치 분석 |
| 5 | 금융 분석 | 금융 카테고리 기사 배치 분석 |
| 6 | 부동산 분석 | 부동산 카테고리 기사 배치 분석 |
| 7 | 종합 산출 | 5개 카테고리 결과 종합 → 리스크 점수/신호 생성 |

**백엔드 연동 (해커톤 당일)**:
- SSE (Server-Sent Events)로 실시간 진행 상황 전달
- 카테고리별 배치 처리로 Claude API 호출 분산
- 특정 카테고리 실패 시 해당 단계만 "실패" 표시하고 계속 진행
...
| `components/news/news-page.tsx` | 클라이언트 컴포넌트 (검색/필터/분석 상태) |
| `components/news/news-filter-bar.tsx` | 키워드 검색 + 카테고리 필터 |
| `components/news/news-list.tsx` | 뉴스 목록 래퍼 |
| `components/news/news-card.tsx` | 개별 뉴스 카드 |
| `components/news/analysis-control-panel.tsx` | 분석 제어 패널 (기간/카테고리/시작) |
| `components/news/analysis-progress-modal.tsx` | 분석 진행 모달 (진행률/단계/완료) |

---

### Phase 6: 차별화 기능 UI (AI 대화형 질의 + 위기 연쇄 반응 맵)

> **참고**: 기존 "지역별 현황 탭"은 위기 신호 탭의 지역 지도로 통합되었습니다.
> 지역 지도 구현 상세는 Phase 4의 "지역 지도 연동" 섹션을 참조하세요.
...
**목표**: 제공된 실제 뉴스 JSON 데이터를 Claude API로 분석하여 7개 파생 JSON을 생성하고 서비스 완성

#### 뉴스 -> JSON 생성 파이프라인 (핵심 아키텍처)

제공되는 원본 뉴스 기사 JSON이 유일한 입력이며, Claude API 3단계 호출을 통해 화면별 데이터를 생성한다.

```
원본 뉴스 기사 JSON (해커톤 당일 제공)
    |
    v
[호출 1] 뉴스 전처리 - classifyArticles()
    |   프롬프트: 6-1
    |   입력: 원본 기사 (title, content, publishedAt, section)
    |   작업: 카테고리 분류, 요약 생성, 키워드 추출, 지역 식별
    |
    +---> news.json (NewsArticle[])
    |
    v
[호출 2] 종합 분석 - analyzeRisks()
    |   프롬프트: 6-2 + 6-3 통합
    |   입력: 분류된 기사 전체 (호출 1 결과)
    |   작업: 신호 감지, 점수 산출, 지역 분석, 인과관계 판별
    |
    +---> signals.json      (Signal[])
    +---> dashboard.json    (DashboardData) *scoreHistory는 기존 히스토리 유지 + overallScore만 교체
    +---> regions.json      (RegionsData - 17개 시도별)
    +---> crisis-chain.json (CrisisChainData)
    |
    v
[호출 3] 브리핑 생성 - generateFullBriefing()
    |   프롬프트: 6-4 + 6-6 통합
    |   입력: 호출 2의 분석 결과 (점수, 신호, 추세)
    |   작업: 자연어 브리핑, 하이라이트, 권고, 향후 전망+시나리오
    |
    +---> briefing.json (BriefingData, forecast 포함)

[별도 유지 - 뉴스 파생 아님]
    policies.json      사전 정의 정책 DB (URL/연락처 최신화만)
    chat-examples.json  AI 대화는 [5]에서 Claude API 직접 호출로 전환
```

| 호출 | 함수명 | 프롬프트 | 생성 JSON | 비고 |
|------|--------|---------|----------|------|
| 1 | classifyArticles() | 6-1 | news.json | 기사 수에 따라 배치 분할 가능 |
| 2 | analyzeRisks() | 6-2 + 6-3 | signals + dashboard + regions + crisis-chain | 전체 맥락을 한번에 분석하여 일관성 확보 |
| 3 | generateFullBriefing() | 6-4 + 6-6 | briefing.json | 호출 2 결과 의존, 자연어 품질 위해 별도 호출 |

**anthropic.ts 함수 명세** (`lib/api/anthropic.ts`):

```ts
// 호출 1: 뉴스 전처리
classifyArticles(rawArticles: RawNewsArticle[]): Promise<NewsArticle[]>

// 호출 2: 종합 분석 (4개 JSON 동시 생성)
analyzeRisks(articles: NewsArticle[]): Promise<{
  signals: Signal[];
  dashboard: Omit<DashboardData, 'scoreHistory'>;  // scoreHistory는 별도 처리
  regions: RegionsData;
  crisisChain: CrisisChainData;
}>

// 호출 3: 브리핑 생성 (호출 2 결과 기반)
generateFullBriefing(analysisResult: AnalyzeRisksResult): Promise<BriefingData>

// 파이프라인 오케스트레이션 (위 3개를 순차 실행)
runAnalysisPipeline(rawArticles: RawNewsArticle[]): Promise<AllGeneratedData>
```

**scoreHistory 처리 전략**:
- 시계열 데이터(90일분)는 단일 시점 뉴스 분석으로 생성 불가
- 해커톤 전략: 기존 목업 히스토리를 유지하되, 최신 overallScore를 마지막 항목으로 추가/교체
- 향후 운영 시: 매 분석 실행마다 기존 히스토리에 새 점수를 append하여 누적
...
[3] Claude API 연결 - 뉴스 -> JSON 생성 파이프라인 (40분) [핵심]

    3-A. 파이프라인 기반 구조 구축
      - lib/api/anthropic.ts에 Claude API 클라이언트 활성화 (주석 해제)
      - runAnalysisPipeline() 오케스트레이션 함수 구현

    3-B. 호출 1: 뉴스 전처리 (news.json 생성)
      - classifyArticles() 함수 구현
      - 프롬프트 6-1 적용 (카테고리 분류 + 요약 + 키워드 + 지역)
      - 원본 기사 -> NewsArticle[] 변환 및 검증
    ...
    3-C. 호출 2: 종합 분석 (4개 JSON 동시 생성)
      ...
      - 출력 JSON 검증:
        * signals.json (위기 신호 목록)
        * dashboard.json (종합 점수 + 카테고리별 점수/추세)
        * regions.json (17개 시도별 점수/이슈)
        * crisis-chain.json (카테고리 간 연쇄 관계)
      - scoreHistory: 기존 목업 히스토리 유지 + overallScore만 교체

    3-D. 호출 3: 브리핑 생성 (briefing.json)
      - generateFullBriefing() 함수 구현 (호출 2 결과 기반)
      - 프롬프트 6-4 + 6-6 통합 적용 (브리핑 + 전망)
      - BriefingData 타입 준수 검증 (forecast 포함)

[4] 페이지 데이터 소스 전환 + 스트리밍 (15분)
    - 각 페이지의 mock import -> 파이프라인 결과 데이터로 교체
    - /api/briefing에서 스트리밍 응답 -> 타이핑 효과 연결
...
[6] 정책 데이터 보완 (10분)
    ...
    - relatedSignals 매핑을 실제 신호 ID로 업데이트
...
[7] 통합 테스트 및 발표 준비 (나머지 시간)
    ...
    - 지역 지도 클릭 -> 필터 연동 정상 동작 확인
    - 리포트 다운로드 기능 정상 동작 확인
    - 실제 데이터 기반 UI 미세 조정
    - 발표 데모 시나리오 리허설
```

**당일 리스크 관리**: 만약 시간이 부족할 경우 파이프라인 호출 3(브리핑)은 Mock 텍스트 유지로 대체 가능. [5] AI 대화형 질의가 가장 높은 발표 임팩트를 가지므로 반드시 연결한다. 파이프라인이 실패해도 기존 Mock JSON으로 전체 데모가 가능하도록 fallback 구조를 유지한다.
...
-- Phase 4 추가: 지역 지도 (위기 신호 탭에 통합) --
components/signals/region-map/region-map.tsx          [지역 지도]
components/signals/region-map/region-tooltip.tsx      [지역 지도]
...
[탭 순회] 상세 기능 시연 (1분 30초)
  ...
  - 위기 신호 탭 지역 지도: 경기도 클릭
    -> 지역별 위기 현황 색상으로 시각화
    -> 경기도 신호 목록으로 필터링 자동 연동
